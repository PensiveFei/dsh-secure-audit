 ...
 * Security signals that look like IPs (0.0.0.0 / ::) are protected from
 * redaction so findings stay actionable, while real PII is always masked. */
function sanitize(base, workspace, text, cap = 600) {
  ...
  for (const [token, placeholder] of protect) {
...
export async function runSecurityAudit(userOptions = {}) {
  ...
  const ctx = { base: baseDir, workspace, baseDir, files, env: process.env };
  ...
  const limitations = [
    ...
    'session-file PII sampling covers up to 10 files; larger directories are bounded by design',
    ...(platform() === 'win32'
