...
/** Values that NAME a secret instead of holding one (env-var reference, flag, number). */
const ENV_REFERENCE_VALUE_RE = /^[A-Z][A-Z0-9_]{2,}$/;
const NON_SECRET_VALUE_RE = /^(?:true|false|null|none|auto|default|inherit|string|number|boolean|-?\d+(?:\.\d+)?)$/i;
/** Keys whose value is a pointer to the secret, never the secret itself. */
const REFERENCE_KEY_RE = /(?:env|envvar|file|path|name|url|uri|id|keyid)$/i;
/** DSH's own credential store: the sanctioned location for provider keys. */
const CREDENTIAL_STORE_FILES = new Set(['.credentials.yaml', '.credentials.yml', '.credentials.json']);
/** Session payload files live under sessions/<workspace>/<session-id>/. */
const SESSION_PAYLOAD_EXT_RE = /\.(?:zstd|zst|gz)$/i;
/** Hard ceiling on sampled session payload files (a huge sampleLimit must not
 *  turn the audit into an unbounded read under the tool timeout). */
...
function checkConfigSecrets(ctx) {
  const findings = [];
  // The DSH credential store is the SANCTIONED location for provider keys:
  // failing it with "move the secrets to the credential store" was circular.
  // It is reported separately (warn, with accurate remediation) instead.
  const storeFindings = [];
  for (const file of ctx.files) {
    if (!CONFIG_EXTS.has(file.slice(file.lastIndexOf('.')))) continue;
    const content = readSafe(file, ctx.maxBytes);
    if (content === null) continue;
    const bucket = CREDENTIAL_STORE_FILES.has(file.split(sep).pop().toLowerCase()) ? storeFindings : findings;
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      // Reference-shaped values are not secrets: apiKeyEnv: DEEPSEEK_API_KEY
      // NAMES an env var, and flags/numbers are settings, not credentials.
      if (ENV_REFERENCE_VALUE_RE.test(value) || NON_SECRET_VALUE_RE.test(value)) continue;
      if (REFERENCE_KEY_RE.test(key)) continue;
      bucket.push({
    ...
    if (findings.length >= 10 && storeFindings.length >= 10) break;
  }
  ...
  if (findings.length === 0) {
    if (storeFindings.length > 0) {
      const storeEvidence = storeFindings
        .map((f) => `${f.rel}:${f.line} ${f.key} set to ${maskSecret('xxxxxxxx')}`)
        .join(' | ');
      return {
        status: 'warn',
        message: `${storeFindings.length} credential(s) stored in the DSH credential store (plaintext at rest)`,
        evidence: sanitize(ctx.base, ctx.workspace, storeEvidence),
        remediation: 'the DSH credential store is the sanctioned location — protect it with OS file permissions and disk encryption instead of moving the values elsewhere',
      };
    }
...
function checkConfigPermissions(ctx) {
  if (process.platform === 'win32') {
    // POSIX mode bits are synthetic on Windows (every path reports 0o666 or
    // 0o444 depending on the read-only flag), so the 0o022 test fired for
    // EVERY path and buried real findings under bogus warnings.
    return {
      status: 'info',
      message: 'file mode bits are synthetic on Windows — ACLs are not inspected by this check',
      remediation: 'inspect ACLs with icacls if you need assurance on Windows',
    };
  }
  const warnings = [];
...
/**
 * Enumerate session payload files under <base>/sessions. DSH 0.1.2 stores one
 * directory per workspace and one per session:
 *   sessions/<workspace-slug>/<session-id>/session.jsonl.zstd
 * Older layouts wrote flat files, so every depth is walked. Directories are
 * never counted as session files (the pre-0.2.10 check did exactly that and
 * reported "1 session file" for one workspace directory).
 * @param {string} sessionsDir sessions root
 * @param {number} maxFiles hard cap on collected files
 * @returns {{files: string[], workspaces: number, truncated: boolean}}
 */
function collectSessionFiles(sessionsDir, maxFiles) {
  const files = [];
  ...
  while (stack.length > 0 && files.length < maxFiles) {
    ...
    for (const ent of entries) {
      ...
      if (ent.isDirectory()) {
        if (depth === 0) workspaces.add(ent.name);
  ...
  return { files: files.sort(), workspaces: workspaces.size, truncated: files.length >= maxFiles };
}

/**
 * Read one session payload, transparently decompressing zstd/zstd and gzip.
 * Returns a reason instead of throwing so the check can REPORT an unreadable
 * payload rather than silently passing over it.
 * @param {string} file payload path
 * @param {number} maxBytes per-file byte cap
 * @returns {{text: string|null, reason: string|null}}
 */
function readSessionPayload(file, maxBytes) {
  let buf;
  try {
    const stat = statSync(file);
    if (!stat.isFile()) return { text: null, reason: 'not-a-file' };
    if (stat.size > maxBytes) return { text: null, reason: 'oversized' };
    buf = readFileSync(file);
  } catch {
    return { text: null, reason: 'unreadable' };
  }
  try {
    if (SESSION_PAYLOAD_EXT_RE.test(file)) {
      if (/\.gz$/i.test(file)) return { text: zlib.gunzipSync(buf).toString('utf8'), reason: null };
      const zstd = typeof zlib.zstdDecompressSync === 'function' ? zlib.zstdDecompressSync : null;
      if (zstd === null) return { text: null, reason: 'zstd-unsupported' };
      return { text: zstd(buf).toString('utf8'), reason: null };
    }
    const text = buf.toString('utf8');
    if (text.includes('\u0000')) return { text: null, reason: 'binary' };
    return { text, reason: null };
  } catch {
    return { text: null, reason: 'decode-failed' };
  }
}
...
function checkSessionsStructure(ctx) {
  ...
  const { files, workspaces, truncated } = collectSessionFiles(sessionsDir, 500);
  const capNote = truncated ? ' (listing capped at 500)' : '';
  return {
    status: 'info',
    message: `found ${files.length} session payload file(s) across ${workspaces} workspace(s)${capNote}`,
    evidence: sanitize(ctx.base, ctx.workspace, `sessions/ contains ${files.length} payload file(s) in ${workspaces} workspace dir(s)`),
  };
}
...
function checkSessionsSensitive(ctx) {
  ...
  let listing;
  try {
    listing = collectSessionFiles(sessionsDir, Math.min(ctx.sampleLimit * 4, 500));
  } catch {
    listing = { files: [], workspaces: 0, truncated: false };
  }
  if (listing.files.length === 0) {
    return { status: 'info', message: 'no session payload files found to inspect' };
  }
  const types = new Map();
  const skipped = new Map();
  let inspected = 0;
  for (const file of listing.files.slice(0, ctx.sampleLimit)) {
    const { text, reason } = readSessionPayload(file, ctx.maxBytes);
    if (text === null) {
      skipped.set(reason, (skipped.get(reason) ?? 0) + 1);
      continue;
    }
    inspected += 1;
    const { findings } = redactText(text);
    for (const f of findings) {
      types.set(f.type, (types.get(f.type) ?? 0) + f.count);
    }
  }
  if (inspected === 0) {
    // Reporting PASS here would claim a PII scan that never happened — the
    // pre-0.2.10 check did exactly that on every 0.1.2 install, whose session
    // payloads are zstd-compressed inside per-session directories.
    const why = [...skipped.entries()].map(([reason, n]) => `${reason}×${n}`).join(', ') || 'no readable payload';
    return {
      status: 'info',
      message: `session payloads could not be inspected (${why}) — PII scan not performed`,
      evidence: sanitize(ctx.base, ctx.workspace, `skipped ${listing.files.length} session payload file(s): ${why}`),
      remediation: 'no action required; this check reports rather than asserts when payloads are unreadable',
    };
  }
  const skippedNote = skipped.size > 0
    ? `; ${[...skipped.values()].reduce((n, v) => n + v, 0)} payload(s) unreadable`
    : '';
  if (types.size === 0) {
    return {
      status: 'pass',
      message: `no PII detected in ${inspected} sampled session file(s)${skippedNote}`,
      evidence: sanitize(ctx.base, ctx.workspace, `sampled ${inspected} session file(s)${skippedNote}`),
    };
  }
  const summary = [...types.entries()].map(([t, n]) => `${t}×${n}`).join(', ');
  return {
    status: 'warn',
    message: `PII detected in ${inspected} sampled session file(s) (${summary})${skippedNote}`,
...
function checkPathsPermissions(ctx) {
  ...
  if (warnings.length === 0) {
    return {
      status: win32 ? 'info' : 'pass',
      message: win32
        ? 'path mode bits are synthetic on Windows — ACLs are not inspected by this check'
        : 'key paths have sane permissions',
    };
  }
  return {
    status: 'warn',
...
export async function runSecurityAudit(userOptions = {}) {
  ...
  const userSampleLimit = Number.isInteger(userOptions.sampleLimit) && userOptions.sampleLimit > 0
    ? Math.min(userOptions.sampleLimit, MAX_SAMPLE_LIMIT)
    : 10;
  ...
  const limitations = [
    ...
    ...(platform() === 'win32'
      ? ['file-permission checks report "info" on Windows: POSIX mode bits are synthetic there and Windows ACLs are not inspected']
      : []),
    'session payloads may be zstd/gzip compressed; unreadable or oversized payloads are reported as info, never as a passing PII scan',
  ];
