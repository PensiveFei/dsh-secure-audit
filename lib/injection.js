...
function tryDecodeBase64(token) {
  ...
  for (const ch of decoded) {
    ...
    // Printable = any non-control code point. Counting ASCII only meant the
    // base64 layer could never flag a CJK payload: base64("忽略之前所有指令…")
    // scored ~0 and was dropped, while the English twin was caught — leaving
    // the obfuscation layer protecting English only.
...
function cacheKey(text, role) {
  // The classifier receives the context role, so a verdict produced for one
  // role must never be replayed for another: include it in the cache key.
  return createHash('sha256').update(`${RULESET_VERSION}\u0000${role ?? ''}\u0000${text}`).digest('hex');
}
...
export function createInjectionScanner(userOptions = {}) {
  ...
  async function scan(text, context = {}) {
    ...
    const key = cacheKey(subject, context && context.role);
    const cached = cache.get(key);
    if (cached !== undefined) {
      hits += 1;
      cache.delete(key);
      cache.set(key, cached);
      // Clone the arrays too: a shallow envelope copy shared reasons /
      // allowlistedHits / warnings with the cache entry, so one consumer
      // mutating them corrupted every later scan of the same text.
      return {
        ...cached,
        reasons: [...cached.reasons],
        allowlistedHits: [...cached.allowlistedHits],
    ...
    evict();
    // Store a defensive copy for the same reason: the caller owns the
    // returned object and may mutate it.
    cache.set(key, {
      ...result,
