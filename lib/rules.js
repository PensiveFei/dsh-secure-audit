...
/**
 * Shannon entropy in bits per character. Used by the high-entropy detector:
 * random secret-like strings (tokens, keys, salts) have high entropy per
 * character, while structured identifiers (UUIDs, hashes, order numbers)
 * drawn from a small alphabet score low and are NOT flagged.
 */
...
/**
 * Heuristic high-entropy detector (conservative by design):
 * - minimum length 24 (short nonces/salts are not flagged);
 * - Shannon entropy >= 4.5 bits/char (UUID ~3.6, hex SHA-256 ~4.0 pass;
 *   base64 / random alphanumeric tokens ~5.2+ are flagged);
 * - at least two character classes (upper/lower/digit/symbol) — a long
 *   all-lowercase word or all-digit run is not flagged.
 */
export function isHighEntropyValue(input, { minLength = 24, minEntropy = 4.5 } = {}) {
  if (typeof input !== 'string' || input.length < minLength) return false;
  if (shannonEntropy(input) < minEntropy) return false;
  const classTests = [/[A-Z]/, /[a-z]/, /[0-9]/, /[^A-Za-z0-9]/];
  let classes = 0;
  for (const re of classTests) {
    if (re.test(input)) classes += 1;
  }
  return classes >= 2;
}
...
export const PII_RULES = Object.freeze({
  ...
  cn_id: {
    ...
    flags: 'g',
    // The pattern's embedded date is structural only (month 01-12, day 01-31);
    // a calendar check rejects impossible dates (Feb 30/31, Apr/Jun/Sep/Nov 31,
    // Feb 29 in a non-leap year) so order numbers embedding them are not
    // false-positived into ID cards.
    validate(match) {
      const year = Number(match.slice(6, 10));
      const month = Number(match.slice(10, 12));
      const day = Number(match.slice(12, 14));
  ...
  high_entropy: {
    label: 'High-entropy token (random secret-like string)',
    // Opt-in type: NOT part of the default redaction mode set (see
    // lib/redact.js). Random-looking strings are masked only when the user
    // explicitly requests modes: ['high_entropy'].
    pattern: '\\b[A-Za-z0-9+/=_-]{24,}\\b',
    flags: 'g',
    validate: (match) => isHighEntropyValue(match),
    mask(match, maskChar) {
      return match.slice(0, 4) + maskChar.repeat(Math.min(match.length - 8, 24)) + match.slice(-4);
    },
  },
});
